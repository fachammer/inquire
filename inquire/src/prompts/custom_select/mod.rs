...
/// Prompt suitable for when you need the user to CustomSelect one option among many.
///
/// The user can CustomSelect and submit the current highlighted option by pressing enter.
///
/// This prompt requires a prompt message and a **non-empty** `Vec` of options to be displayed to the user. The options can be of any type as long as they implement the `Display` trait. It is required that the `Vec` is moved to the prompt, as the prompt will return the CustomSelected option (`Vec` element) after the user submits.
/// - If the list is empty, the prompt operation will fail with an `InquireError::InvalidConfiguration` error.
///
/// This prompt does not support custom validators because of its nature. A submission always CustomSelects exactly one of the options. If this option was not supposed to be CustomSelected or is invalid in some way, it probably should not be included in the options list.
///
...
///   - Prints the CustomSelected option string value by default.
...
///
/// ```no_run
/// use inquire::{error::InquireError, CustomSelect};
///
/// let options: Vec<&str> = vec!["Banana", "Apple", "Strawberry", "Grapes",
///     "Lemon", "Tangerine", "Watermelon", "Orange", "Pear", "Avocado", "Pineapple",
/// ];
///
/// let ans: Result<&str, InquireError> = CustomSelect::new("What's your favorite fruit?", options).prompt();
///
/// match ans {
...
pub struct CustomSelect<'a, T> {
    ...
    /// Starting cursor index of the CustomSelection.
...
impl<'a, T> CustomSelect<'a, T>
where
    T: Display + 'static,
{
    /// String formatter used by default in [CustomSelect](crate::CustomSelect) prompts.
    /// Simply prints the string value contained in the CustomSelected option.
    ///
    /// # Examples
    ///
    /// ```
    /// use inquire::list_option::ListOption;
    /// use inquire::CustomSelect;
    ///
    /// let formatter = CustomSelect::<&str>::DEFAULT_FORMATTER;
    ...
    /// use inquire::CustomSelect;
    ///
    /// let scorer = CustomSelect::<&str>::DEFAULT_SCORER;
    ...
    pub const DEFAULT_HELP_MESSAGE: Option<&'a str> =
        Some("↑↓ to move, enter to CustomSelect, type to filter");

    /// Creates a [CustomSelect] with the provided message and option_fetcher, along with default configuration values.
    pub fn new<O: OptionFetcher<T> + 'static>(message: &'a str, option_fetcher: O) -> Self {
        Self {
            message,
            options_fetcher: Box::new(option_fetcher),
    ...
    ///
    /// Returns the owned object CustomSelected by the user.
    ...
    /// the index of the CustomSelection and the owned object CustomSelected by the user.
    ...
    pub(crate) fn prompt_with_backend<B: SelectBackend>(
        ...
        CustomSelectPrompt::new(self)?.prompt(backend)
    }
}
